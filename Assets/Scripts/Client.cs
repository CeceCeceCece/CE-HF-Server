...
public class Client
{
    ...
    public RigidbodyPlayer rbplayer;
    public TCP tcp;
    public UDP udp;
    ...
    public void SendIntoGame(string _playerName)
    {
        ...
        foreach (Client _client in Server.clients.Values)
        {
            if (_client.rbplayer != null)
            {
                if (_client.id != id)
                {
                    ServerSend.SpawnPlayer(id, _client.rbplayer);
                }
            }
        }
        ...
        foreach (Client _client in Server.clients.Values)
        {
            if (_client.rbplayer != null)
            {
                ServerSend.SpawnPlayer(_client.id, rbplayer);
            }
        }
    ...
    public void Disconnect()
    {
        ...
        ThreadManager.ExecuteOnMainThread(() => {   
            UnityEngine.Object.Destroy(rbplayer.gameObject);
            rbplayer = null;
        });
        tcp.Disconnect();
        udp.Disconnect();
